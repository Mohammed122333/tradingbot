...
def main():
    ...
    oos_date_to_str = kaggle_cfg.get("oos_date_to", kaggle_cfg.get("backtest_date_to", "2026-06-19"))
    ...
    with mp_ctx.Pool(processes=worker_count, initializer=init_worker, initargs=(cached_data_dict, fixed_params)) as pool:
        for method in methods:
            ...
            for idx, combo in enumerate(grid):
                combo_key = f"{idx}_{hash(str(combo))}"
            ...
            for i, res in enumerate(pool.imap_unordered(run_combo, tasks_only, chunksize=chunksize)):
                ...
                is_prof = res and not res.get('is_waste', False) and res.get('trades', 0) >= 1 and res.get('roi', 0.0) > 0.0
                if is_prof:
